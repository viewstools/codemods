...
function transform(rtext) {
  ...
  const next = lines.map((line, i) => {
    ...
    const [_2, props, slotIsNot = '', propsProp = ''] = getProps(value)

    const slot = ['<', slotIsNot, propsProp.replace('.', '')].filter(Boolean)
    return line.replace(props, slot.join(''))
  })
